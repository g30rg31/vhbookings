...
use PHPMailer\PHPMailer\Exception;
	...
	$sql = 'SELECT bookings.id, bookings.customer AS name, DATE_FORMAT(bookings.startdate, "%d %M %Y") AS date, DATE_FORMAT(bookings.startdate, "%H:%i") AS timestart,
		...
		bookings.numrecs, customers.email, bookings.public AS pubdets, bookings.publiccontact AS pubcons 
                FROM bookings INNER JOIN customers ON bookings.customer=customers.name
		where bookings.id = "' . $_POST["id"] . '"';
//echo $sql . PHP_EOL;
...
$filename = "mail/chgreqemail.html";
...
//    $mail->addAttachment('/tmp/image.jpg', 'new.jpg');    //Optional name
    ...
    $mail->Subject = 'Booking Change Request';
    $mail->Body    = $body;
